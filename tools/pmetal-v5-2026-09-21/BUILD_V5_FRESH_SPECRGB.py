...
def patch_v3_blob(blob,idx):
    ...
    assert (old_sample[0]&0x7ff)==0x45, (idx,hex(old_sample[0]))
    assert old_sample[7]==0x00107936 and old_sample[8]==0x0000000a, (idx,'not t10')
    assert old_sample[9]==0x00106000 and old_sample[10]==0x00000001, (idx,'not s1')
    assert all(x==NOP for x in w[t['nop_start']:t['mul']]), (idx,'NOP island changed')
...
readme='''DSRRL Material Response 1.45 — P_Metal Fresh SpecRGB V5

Drop-in runtime candidate based directly on V3 exact EnvSpec.

Root cause fixed:
V3 sampled the PTDE SpecRGB sidecar correctly, but that sample register was overwritten before the final material multiply. The final EnvSpec material tail therefore consumed a stale DSR material-workflow temporary (r11/r10/r9) instead of live PTDE SpecRGB.
