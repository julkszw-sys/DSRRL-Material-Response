...
def patch(blob, idx):
    assert validate(blob), f'input checksum invalid {idx}'
...
def main():
    root=Path('/mnt/data/v3_work/dxbc'); out=Path('/mnt/data/v4_domain_work/dxbc'); out.mkdir(parents=True,exist_ok=True)
    report={}
    for i in range(78):
        src=(root/f'{i:02d}.dxbc').read_bytes()
        assert validate(src), f'bad existing checksum {i}'
        dst=patch(src,i) if i in POS else src
        (out/f'{i:02d}.dxbc').write_bytes(dst)
        diffs=[j for j,(a,b) in enumerate(zip(src,dst)) if a!=b]
        report[str(i)]={'size':len(dst),'sha256_before':hashlib.sha256(src).hexdigest(),'sha256_after':hashlib.sha256(dst).hexdigest(),'diff_bytes':len(diffs),'checksum_valid':validate(dst)}
    Path('/mnt/data/v4_domain_work/DXBC_PATCH_AUDIT.json').write_text(json.dumps(report,indent=2))
    print(json.dumps({k:v for k,v in report.items() if v['diff_bytes']},indent=2))
