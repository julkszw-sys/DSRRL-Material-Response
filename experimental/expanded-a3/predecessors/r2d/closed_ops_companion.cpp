...
static void log_plan_state(const char *tag,u32 pi,u32 reason){
    ...
    q=put_s(q,"DSRRL R2D ");q=put_s(q,tag);q=put_s(q," plan=0x");q=put_h32(q,pi);
...
static void log_plan_bind(u32 pi){
    ...
    q=put_s(q,"DSRRL R2D BIND plan=0x");q=put_h32(q,pi);
...
static bool on_create_pipeline(void *,u64,u32 n,const PipelineSubobject *sub){
    ...
    ps->code=replacement;
    if(g_log && __atomic_exchange_n(&g_active_logged,1u,__ATOMIC_ACQ_REL)==0u) g_log(g_addon_module,3,"DSRRL Closed Operators 1.45 R2D: FIRST CLOSED BRIDGE ACTIVE.");
    return true;
}
...
extern "C" __declspec(dllexport) const char *NAME="DSRRL PTDE Closed Operators 1.45 R2D";
extern "C" __declspec(dllexport) const char *AUTHOR="DSR Restored Lighting";
extern "C" __declspec(dllexport) const char *DESCRIPTION="R2D deterministic materialization diagnostic: identical R1 240-plan shader math with persistent per-plan replacement storage and first-bind telemetry.";
...
extern "C" __declspec(dllexport) bool AddonInit(void *addon_module,void *reshade_module){
    ...
    if(g_log)g_log(addon_module,3,"DSRRL Closed Operators 1.45 R2D: armed 240 plans; R1 math unchanged; persistent per-plan storage; no transient scratch-slot fail-open; first-bind telemetry enabled.");
    return true;
}
