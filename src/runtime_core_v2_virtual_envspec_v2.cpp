    ...
    reshade::api::resource source)
{
    ...
    replacement.packed_index = *packed_index;
    replacement.probe_index = *packed_index / k_slots;
    replacement.slot = 2;
    ...
    HMODULE reshade_module)
{
    ...
    try_bind_material_response_companion();

    if (!load_pack(addon_module)) {
        reshade::log::message(
            reshade::log::level::error,
            "[DSRRL VIRTUAL ENVSPEC V2] canonical PackedGI load/SHA failed; fail-open");
    }
    else {
        reshade::log::message(
            reshade::log::level::info,
            "[DSRRL VIRTUAL ENVSPEC V2] canonical PackedGI PASS; slot2 virtualization armed");
    }
