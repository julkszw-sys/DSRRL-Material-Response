...
constexpr std::uint32_t k_pmetal_local_min = 9u;
constexpr std::uint32_t k_pmetal_local_max = 11u;
constexpr std::uintptr_t k_mr_tls_index_rva = 0x107B50;
constexpr std::uintptr_t k_mr_prepare_gate_rva = 0x19ABD3;

struct material_response_companion {
...
material_response_companion g_companion;

std::atomic<bool> g_pack_ready{false};
std::atomic<std::uint64_t> g_native_desc_candidates{0};
std::atomic<std::uint64_t> g_native_initial_missing{0};
std::atomic<std::uint64_t> g_native_hash_hits{0};
std::atomic<std::uint64_t> g_native_hash_misses{0};
...
std::atomic<std::uint64_t> g_companion_attempts{0};
std::atomic<std::uint64_t> g_companion_found{0};
std::atomic<std::uint64_t> g_pmetal_hits{0};
std::atomic<std::uint64_t> g_route_reject{0};
...
bool has_supported_prepare_signature(const std::uint8_t *p) noexcept
{
    static constexpr std::uint8_t current_sig[] = {
        0x8B, 0x49, 0x14, 0x83, 0xF9, 0x17
    };
    static constexpr std::uint8_t legacy_sig[] = {
        0x8B, 0x49, 0x14, 0x85, 0xC9
    };

    return std::memcmp(p, current_sig, sizeof(current_sig)) == 0 ||
           std::memcmp(p, legacy_sig, sizeof(legacy_sig)) == 0;
}

bool try_bind_material_response_companion_once()
{
    if (g_companion.ready)
        return true;

    ++g_companion_attempts;
    ...
    if (Module32FirstW(snapshot, &entry)) {
        do {
            ...
            if (module == nullptr ||
                entry.modBaseSize <= k_mr_prepare_gate_rva + 16u ||
                entry.modBaseSize <= k_mr_tls_index_rva + sizeof(DWORD))
                continue;

            auto *name_export = reinterpret_cast<const char *const *>(
                GetProcAddress(module, "NAME"));
            if (name_export == nullptr || *name_export == nullptr)
                continue;

            const std::string name(*name_export);
            if (name.find("DSRRL Material Response 1.45") ==
                std::string::npos)
                continue;

            const auto base = reinterpret_cast<std::uintptr_t>(module);
            const auto *prepare = reinterpret_cast<const std::uint8_t *>(
                base + k_mr_prepare_gate_rva);
            if (!has_supported_prepare_signature(prepare))
                continue;

            const DWORD tls_index = *reinterpret_cast<const DWORD *>(
                base + k_mr_tls_index_rva);
            if (tls_index == TLS_OUT_OF_INDEXES)
                continue;

            g_companion.module = module;
            g_companion.base = base;
            g_companion.tls_index = tls_index;
            g_companion.ready = true;
            ++g_companion_found;
            found = true;
            break;
...
std::filesystem::path addon_root(HMODULE addon_module)
{
...
{
    return addon_root(addon_module) / L"DSRRL" / L"EnvSpec" /
        L"PackedGI" / L"PTDE_GI_ENVSPEC_PACK_RGBA.bin";
}

std::filesystem::path fingerprint_path_from_module(HMODULE addon_module)
{
    return addon_root(addon_module) / L"DSRRL" / L"EnvSpec" /
        L"PackedGI" / L"DSR_GI_ENVSPEC_SHA256.csv";
}

bool load_native_fingerprints(HMODULE addon_module)
{
    std::ifstream file(fingerprint_path_from_module(addon_module));
    if (!file)
        return false;

    g_native_hash_to_probe.clear();
    ...
    while (std::getline(file, line)) {
        ...
        for (std::size_t i = 0; i < 4; ++i) {
            ...
            if (comma == std::string::npos)
                return false;
            fields[i] = line.substr(begin, comma - begin);
            begin = comma + 1;
        }
        fields[4] = line.substr(begin);

        const std::uint32_t probe =
            static_cast<std::uint32_t>(std::stoul(fields[0]));
        if (probe >= k_probe_count || fields[4].size() != 64)
            return false;

        const auto [_, inserted] =
            g_native_hash_to_probe.emplace(fields[4], probe);
        if (!inserted)
            return false;
        ++rows;
    }

    return rows == k_probe_count;
}
...
bool load_pack(HMODULE addon_module)
{
    ...
    if (!load_native_fingerprints(addon_module)) {
        g_pack.clear();
        return false;
    }
...
bool extract_native_dsr_cube_bytes(
    const reshade::api::resource_desc &desc,
    const reshade::api::subresource_data *initial_data,
    std::vector<std::uint8_t> &out)
{
    if (desc.type != reshade::api::resource_type::texture_2d ||
        desc.texture.width != k_target_size ||
        desc.texture.height != k_target_size ||
        desc.texture.depth_or_layers != k_faces ||
        desc.texture.levels != k_target_levels ||
        desc.texture.samples != 1 ||
        desc.texture.format != reshade::api::format::bc6h_ufloat)
        return false;

    ++g_native_desc_candidates;

    if (initial_data == nullptr) {
        ++g_native_initial_missing;
        return false;
    }
    ...
    for (std::uint32_t face = 0; face < k_faces; ++face) {
        for (std::uint32_t level = 0; level < k_target_levels; ++level) {
            const std::uint32_t subresource =
                face * k_target_levels + level;
            const reshade::api::subresource_data &src =
                initial_data[subresource];
            if (src.data == nullptr)
                return false;

            const std::uint32_t size =
                std::max(1u, k_target_size >> level);
            const std::uint32_t blocks =
                std::max(1u, (size + 3u) / 4u);
            const std::uint32_t tight_row = blocks * 16u;
            ...
            out.resize(old_size +
                static_cast<std::size_t>(blocks) * tight_row);
            std::uint8_t *dst = out.data() + old_size;

            for (std::uint32_t row = 0; row < blocks; ++row) {
                std::memcpy(
                    dst + static_cast<std::size_t>(row) * tight_row,
                    src_bytes + static_cast<std::size_t>(row) * row_pitch,
                    tight_row);
            }
        }
    }

    return out.size() == 524352;
}

std::optional<std::uint32_t> match_native_dsr_probe(
    const std::vector<std::uint8_t> &cube)
{
    std::array<std::uint8_t, 32> digest{};
    if (!sha256(cube, digest))
        return std::nullopt;

    const std::string hex = hex_string(digest.data(), digest.size());
    const auto it = g_native_hash_to_probe.find(hex);
    if (it == g_native_hash_to_probe.end()) {
        ++g_native_hash_misses;
        return std::nullopt;
    }
    ...
    replacement_resource &out)
{
    ...
    // Mip 3 is the exact 32x32 PTDE decoded-radiance topology.
    // Mips 0..2 are only spatial upscales; mips 4..7 are a standard
    // 2x2 radiance pyramid. No gain/compensation is introduced.
    std::array<std::vector<float3>, k_target_subresources> linear;

    for (std::uint32_t face = 0; face < k_faces; ++face) {
        const std::uint32_t anchor_level = 3;
        const std::uint32_t anchor_subresource =
            face * k_target_levels + anchor_level;
        auto &anchor = linear[anchor_subresource];
        anchor.resize(
            static_cast<std::size_t>(k_source_size) * k_source_size);

        for (std::uint32_t y = 0; y < k_source_size; ++y) {
            for (std::uint32_t x = 0; x < k_source_size; ++x) {
                const float4 p = load_rgba(source_cube, face, x, y);
                const float alpha = std::max(p.a, 1.0f / 255.0f);
                anchor[static_cast<std::size_t>(y) * k_source_size + x] =
                    float3{p.r / alpha, p.g / alpha, p.b / alpha};
            }
        }
        ...
        for (int level = 2; level >= 0; --level) {
            ...
            const std::uint32_t subresource =
                face * k_target_levels + static_cast<std::uint32_t>(level);
            auto &pixels = linear[subresource];
            ...
            for (std::uint32_t y = 0; y < size; ++y) {
                ...
                for (std::uint32_t x = 0; x < size; ++x) {
                    ...
                    pixels[static_cast<std::size_t>(y) * size + x] =
                        sample_ptde_decoded(
                            source_cube, face, source_x, source_y);
                }
            }
        }

        for (std::uint32_t level = 4; level < k_target_levels; ++level) {
            const std::uint32_t size = k_target_size >> level;
            const std::uint32_t prev_size = size * 2u;
            const std::uint32_t subresource =
                face * k_target_levels + level;
            const std::uint32_t prev_subresource =
                face * k_target_levels + (level - 1u);
            auto &pixels = linear[subresource];
            const auto &prev = linear[prev_subresource];
            pixels.resize(static_cast<std::size_t>(size) * size);

            for (std::uint32_t y = 0; y < size; ++y) {
                for (std::uint32_t x = 0; x < size; ++x) {
                    const std::size_t i00 =
                        static_cast<std::size_t>(y * 2u) * prev_size + x * 2u;
                    const std::size_t i10 = i00 + 1u;
                    const std::size_t i01 = i00 + prev_size;
                    const std::size_t i11 = i01 + 1u;

                    const float3 &a = prev[i00];
                    const float3 &b = prev[i10];
                    const float3 &c = prev[i01];
                    const float3 &d = prev[i11];

                    pixels[static_cast<std::size_t>(y) * size + x] =
                        float3{
                            0.25f * (a.r + b.r + c.r + d.r),
                            0.25f * (a.g + b.g + c.g + d.g),
                            0.25f * (a.b + b.b + c.b + d.b)};
                }
            }
        }
        ...
        for (std::uint32_t level = 0; level < k_target_levels; ++level) {
            ...
            const std::uint32_t subresource =
                face * k_target_levels + level;
            const auto &src = linear[subresource];
            auto &pixels = storage[subresource];
            pixels.resize(src.size());

            for (std::size_t i = 0; i < src.size(); ++i)
                pixels[i] = pack_r11g11b10(src[i]);

            subresources[subresource].data = pixels.data();
            subresources[subresource].row_pitch =
    ...
    reshade::api::resource source)
{
    ...
    if (!extract_native_dsr_cube_bytes(desc, initial_data, cube))
        return;

    const std::optional<std::uint32_t> probe_index =
        match_native_dsr_probe(cube);
    ...
    replacement.probe_index = *probe_index;
    replacement.packed_index = *probe_index * k_slots + 2u;
    replacement.slot = 2;

    std::lock_guard lock(g_mutex);
    g_replacements_by_resource.emplace(source.handle, replacement);
}
    ...
    reshade::api::resource_view source_view)
{
    ...
    g_source_resource_by_view[source_view.handle] = source.handle;
}
    ...
    reshade::api::resource_view source_view)
{
    ...
    std::lock_guard lock(g_mutex);
    g_source_resource_by_view.erase(source_view.handle);
}
    ...
    reshade::api::resource_view source_view)
{
    ...
    {
        std::lock_guard lock(g_mutex);
        const auto vit = g_source_resource_by_view.find(source_view.handle);
        if (vit == g_source_resource_by_view.end())
            return std::nullopt;
    ...
    const std::uint8_t *cube =
        g_pack.data() +
        static_cast<std::size_t>(record.packed_index) *
            k_source_bytes_per_cube;
...
void log_snapshot(const runtime_snapshot &snapshot)
{
    ...
    if (!g_companion.ready)
        try_bind_material_response_companion_once();

    char line[1024] = {};
    std::snprintf(
        line,
        sizeof(line),
        "[DSRRL VIRTUAL ENVSPEC V3.1] P=%llu PACK=%u native_desc=%llu "
        "initial_missing=%llu hash_hit=%llu hash_miss=%llu "
        "companion=%u attempts=%llu pmetal=%llu route_reject=%llu host_reject=%llu tls_null=%llu "
        "virtual=%llu create_fail=%llu t12=%llu t14=%llu "
        "LIVE res=%llu view=%llu cmd=%llu",
        static_cast<unsigned long long>(present),
        g_pack_ready.load() ? 1u : 0u,
        static_cast<unsigned long long>(g_native_desc_candidates.load()),
        static_cast<unsigned long long>(g_native_initial_missing.load()),
        static_cast<unsigned long long>(g_native_hash_hits.load()),
        static_cast<unsigned long long>(g_native_hash_misses.load()),
        g_companion.ready ? 1u : 0u,
        static_cast<unsigned long long>(g_companion_attempts.load()),
        static_cast<unsigned long long>(g_pmetal_hits.load()),
        static_cast<unsigned long long>(g_route_reject.load()),
...
extern "C" __declspec(dllexport) const char *NAME =
    "DSRRL PTDE Virtual EnvSpec V3.1";
extern "C" __declspec(dllexport) const char *DESCRIPTION =
    "Exact P_Metal native-probe PTDE-to-DSR-topology virtual EnvSpec V3.1; no hot-path module scan.";
    ...
    HMODULE reshade_module)
{
    ...
    if (!load_pack(addon_module)) {
        reshade::log::message(
            ...
            "[DSRRL VIRTUAL ENVSPEC V3.1] canonical PackedGI load/SHA failed; fail-open");
    }
    else {
        reshade::log::message(
            reshade::log::level::info,
            "[DSRRL VIRTUAL ENVSPEC V3.1] canonical PackedGI PASS; native DSR probe fingerprint bridge armed");
    }
