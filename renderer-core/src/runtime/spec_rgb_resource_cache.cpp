...
namespace {
spec_rgb_bind_plan fail_open(const operators::resource_bridges::spec_rgb_decision &route, spec_rgb_resource_handle stock_t1) noexcept { spec_rgb_bind_plan out; out.route=route; out.preserved_t1_srv=stock_t1; return out; }
}

spec_rgb_resource_result spec_rgb_resource_cache::register_exact(const std::u16string &logical_name, spec_rgb_resource_handle stock_t1_srv, spec_rgb_resource_handle ptde_t10_srv)
{
    if (logical_name.empty()) return spec_rgb_resource_result::fail_open_invalid_identity;
    ...
    if(stock_owner!=name_by_stock_.end() && stock_owner->second!=logical_name){
        ...
        if(previous!=by_name_.end()){ name_by_companion_.erase(previous->second.ptde_t10_srv); previous->second.stock_t1_srv=0; previous->second.ptde_t10_srv=0; previous->second.ambiguous=true; }
        name_by_stock_.erase(stock_owner); ambiguous_stock_.insert(stock_t1_srv); return spec_rgb_resource_result::fail_open_conflicting_stock_binding;
    }
    const auto found=by_name_.find(logical_name);
    if(found==by_name_.end()){ by_name_.emplace(logical_name,entry{logical_name,stock_t1_srv,ptde_t10_srv,false}); name_by_stock_[stock_t1_srv]=logical_name; name_by_companion_[ptde_t10_srv]=logical_name; return spec_rgb_resource_result::registered; }
    entry &existing=found->second;
    if(existing.ambiguous) return spec_rgb_resource_result::fail_open_ambiguous_identity;
    if(existing.stock_t1_srv==stock_t1_srv && existing.ptde_t10_srv==ptde_t10_srv) return spec_rgb_resource_result::already_registered;
    name_by_stock_.erase(existing.stock_t1_srv); name_by_companion_.erase(existing.ptde_t10_srv); existing.stock_t1_srv=0; existing.ptde_t10_srv=0; existing.ambiguous=true; return spec_rgb_resource_result::fail_open_conflicting_companion;
}

spec_rgb_bind_plan spec_rgb_resource_cache::plan_bind(const spec_rgb_bind_request &request) const
{
    auto route=operators::resource_bridges::evaluate_spec_rgb_route(request.route); auto out=fail_open(route,request.currently_bound_t1);
    if(route.action!=operators::resource_bridges::spec_rgb_action::bind_ptde_t10_rgb || request.logical_name.empty() || request.currently_bound_t1==0) return out;
    std::lock_guard<std::mutex> lock(mutex_);
...
void spec_rgb_resource_cache::erase_stock(spec_rgb_resource_handle stock_t1_srv)
{
    if(stock_t1_srv==0) return; std::lock_guard<std::mutex> lock(mutex_); ambiguous_stock_.erase(stock_t1_srv);
    const auto owner=name_by_stock_.find(stock_t1_srv); if(owner==name_by_stock_.end()) return;
    const auto found=by_name_.find(owner->second); if(found!=by_name_.end() && found->second.stock_t1_srv==stock_t1_srv){ name_by_companion_.erase(found->second.ptde_t10_srv); by_name_.erase(found); }
