...
constexpr std::array<operator_contract, operator_count> k_catalog = {{
    ...
    {operator_id::hemdir3, "surface.hemdir3", "project.branch.renderer_edition_hemdir3_source_join_contract", "HemDir3 D1/D2/D3 + U/L source", canonical_status::confirmed, port_state::blocked, carrier_kind::constant_buffer, require_receiver | require_producer | require_consumer, true},
    ...
    {operator_id::env_spec, "surface.envspec_legacy", "project.branch.renderer_edition_envspec_exact_legacy_bridge_contract_v1", "EnvSpec exact legacy bridge", canonical_status::confirmed, port_state::blocked, carrier_kind::hybrid, require_receiver | require_material | require_resource | require_consumer, true},
    ...
    {operator_id::env_diffuse, "surface.envdiffuse", "renderer.ptde.envdiffuse.end_to_end_forward_v1", "EnvDiffuse full causal operator", canonical_status::high_confidence, port_state::blocked, carrier_kind::hybrid, require_receiver | require_resource | require_producer | require_consumer, true},
    ...
    {operator_id::local_specular_legacy, "surface.local_specular_legacy", "renderer.cross.pointlight.ptde_specular_branch_output_cut", "Legacy local PointLight specular", canonical_status::confirmed, port_state::blocked, carrier_kind::hybrid, require_receiver | require_material | require_producer | require_consumer, true},
    {operator_id::subsurface, "bridge.subsurface", "project.renderer.material_response_single_addon_subsurf_normal_diffuse_v1", "PTDE Subsurface route island", canonical_status::confirmed, port_state::off, carrier_kind::hybrid, require_receiver | require_material | require_resource | require_consumer, true},
    ...
    {operator_id::terminal_sat_rgba, "surface.terminal_sat_rgba", "renderer.diagnostic.combined_rgba_terminal_sat_expansion_candidate", "Terminal RGBA SAT expansion", canonical_status::hypothesis, port_state::diagnostic, carrier_kind::shader, require_receiver | require_consumer, true},
    ...
    {operator_id::faceeye_shadow_legacy, "surface.faceeye_shadow_legacy", "renderer.ptde.faceeye_shadow_operator_v1", "PTDE FaceEye legacy shadow/environment operator", canonical_status::confirmed, port_state::blocked, carrier_kind::shader, require_receiver | require_consumer, true},
