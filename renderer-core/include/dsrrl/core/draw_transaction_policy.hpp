...
constexpr bool upper_lower_native_readback_skip_allowed(
    const upper_lower_readback_skip_shape &shape) noexcept
{
    const auto ul = operator_bit(operator_id::upper_lower);
    ...
    for (std::size_t i = 0u; i < operator_count; ++i) {
        const auto op = static_cast<operator_id>(i);
        const auto bit = operator_bit(op);
        if (bit == ul || (shape.owners & bit) == 0u)
            continue;
