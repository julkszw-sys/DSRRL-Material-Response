...
struct faceeye_auxiliary_snapshot_descriptor { std::array<bool,26> register_present{}; std::array<std::uint16_t,26> producer_offsets{}; bool immutable_draw_local=false; bool ptde_dsr_value_homology_verified=false; };
bool validate_faceeye_auxiliary_snapshot(const faceeye_auxiliary_snapshot_descriptor &snapshot) noexcept;
