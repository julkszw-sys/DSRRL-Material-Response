...
enum bloom_scene_writer_class : std::uint32_t {
    bloom_writer_none = 0u,
    bloom_writer_main_flver_material = 1u << 0,
    bloom_writer_fx_sfx_target_scene = 1u << 1
};

inline constexpr std::uint32_t bloom_known_writer_classes =
    bloom_writer_main_flver_material |
    bloom_writer_fx_sfx_target_scene;
...
enum class bloom_fx_material_route_proof : std::uint8_t {
    unknown = 0,
    authored_spx_mtd_identity_closed,
    fx_entity_material_binding_closed,
    collector_material_binding_closed
};

enum class bloom_fx_blend_semantics_proof : std::uint8_t {
    unknown = 0,
    authored_blend_mode_closed,
    render_state_tuple_closed
};

enum class bloom_fx_identity_transport_proof : std::uint8_t {
    unknown = 0,
    authored_identity_token_closed,
    adapter_entity_token_closed,
    collector_draw_token_closed
};

enum class bloom_fx_identity_join_proof : std::uint8_t {
    unknown = 0,
    same_authored_material_closed,
    same_runtime_material_instance_closed,
    same_collector_draw_closed
};
...
enum class bloom_scene_bridge_result : std::uint8_t {
    ...
    fx_identity_transport_not_closed,
    fx_identity_join_not_closed,
...
struct bloom_scene_bridge_carrier {
    ...
    bloom_scene_construction_strategy strategy =
        bloom_scene_construction_strategy::unknown;

    std::uint32_t proven_writer_classes =
        bloom_writer_none;
    bool writer_set_exhaustiveness_proven = false;

    bloom_writer_order_proof writer_order =
    ...
    bloom_fx_material_route_proof fx_material_route =
        bloom_fx_material_route_proof::unknown;
    bloom_fx_blend_semantics_proof fx_blend_semantics =
        bloom_fx_blend_semantics_proof::unknown;
    bloom_fx_identity_transport_proof fx_identity_transport =
        bloom_fx_identity_transport_proof::unknown;
    bloom_fx_identity_join_proof fx_identity_join =
    ...
    const bloom_scene_bridge_carrier &c) noexcept
{
    ...
    if (!c.writer_set_exhaustiveness_proven ||
        (c.proven_writer_classes & bloom_known_writer_classes) !=
            bloom_known_writer_classes)
        return bloom_scene_bridge_result::writer_class_coverage_incomplete;

    if (c.writer_order !=
        bloom_writer_order_proof::execution_order_closed)
    ...
    if (c.fx_material_route !=
        bloom_fx_material_route_proof::collector_material_binding_closed)
        return bloom_scene_bridge_result::fx_material_route_not_closed;

    if (c.fx_blend_semantics !=
        bloom_fx_blend_semantics_proof::render_state_tuple_closed)
        return bloom_scene_bridge_result::fx_blend_semantics_not_closed;

    if (c.fx_identity_transport !=
        bloom_fx_identity_transport_proof::collector_draw_token_closed)
        return bloom_scene_bridge_result::fx_identity_transport_not_closed;

    if (c.fx_identity_join !=
        bloom_fx_identity_join_proof::same_collector_draw_closed)
        return bloom_scene_bridge_result::fx_identity_join_not_closed;
    ...
    if (c.strategy !=
            bloom_scene_construction_strategy::pre_storage_history_replay &&
        c.strategy !=
