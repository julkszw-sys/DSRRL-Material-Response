...
// PTDE Bloom pass 0x04 is role-stratified. The first invocation belongs to
// packed-depth production. The second invocation is a distinct color-transfer
// edge from the authenticated Q8 scene history into the 256x180 RGBA ladder.
// Stock DSR's depth-only pass 0x04 cannot authenticate that second role merely
// because the numeric pass id matches.
...
{
    if (!c.q8_scene_1024x720_ready ||
        !c.packed_depth_256x180_ready ||
        !c.fixed_rgba_256x180_ready ||
        !c.fixed_rgba_128x90_ready)
    ...
    if (!c.pass18_brightpass_ready ||
        !c.pass19_blur_h_ready ||
        !c.pass1a_blur_v_ready ||
        !c.hdr_t1_consumes_rgba_128x90)
