    ...
    HMODULE reshade_module)
{
    ...
    if (!flver_hooks || !bloom_fx_hooks ||
        !hemdir3_mode_hooks || !upper_lower_hooks) {
        reshade::log::message(
            reshade::log::level::warning,
            "[DSRRL PERF DIAG G_FLVER_OPERATOR_NO_TEXTURE] One or more native FLVER-dependent hooks failed to arm.");
    }

    reshade::log::message(
        reshade::log::level::warning,
        "[DSRRL PERF DIAG G_FLVER_OPERATOR_NO_TEXTURE] FLVER + Bloom/HemDir3/UpperLower native hooks active; texture identity and ReShade event/resource layers disabled. Audit marker: BLOOM_FX ww_diag_join; Bloom FX draw transport FAIL-OPEN where disabled by this diagnostic.");
...
void AddonUninit(
    ...
    HMODULE reshade_module)
{
    // PERF DIAG FLVER_OPERATOR_NO_TEXTURE: no ReShade callbacks were registered.
