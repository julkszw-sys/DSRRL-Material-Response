...
int main()
{
    ...
    // U/L native readback waiver is deliberately narrower than owner
    // membership: only PS + one b13 carrier may use it. Mixed draw-required
    // islands and any resource mutation retain full native verification.
    const auto ul_bit =
        core::operator_bit(
            core::operator_id::upper_lower);
    const auto sat_bit =
    ...
    CHECK(core::upper_lower_native_readback_skip_allowed(
        ul_fast));

    auto ul_with_create_time = ul_fast;
    ul_with_create_time.owners |= sat_bit;
    ul_with_create_time.shader_owners |= sat_bit;
    CHECK(core::upper_lower_native_readback_skip_allowed(
        ul_with_create_time));
