...
int main()
{
    ...
    request.currently_bound_t1 = 0x102u; plan = cache.plan_bind(request); CHECK(!plan.activate);
    request = ready_request(); request.route.actual_material_verified = false; plan = cache.plan_bind(request); CHECK(!plan.activate);
    ...
    request = ready_request(); CHECK(!cache.plan_bind(request).activate);
    cache.clear();
    ...
    cache.erase_stock(0x301u); CHECK(cache.size() == 1u);
