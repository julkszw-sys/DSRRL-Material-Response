...
int main()
{
    ...
    const auto bloom_contract=core::find_operator_contract(core::operator_id::post_bloom);
    ...
    const auto hdr_contract=core::find_operator_contract(core::operator_id::post_hdr);
    ...
    auto gate=core::evaluate_operator_activation(features,core::operator_id::post_bloom,activation);
    CHECK(gate.state==core::island_state::fail_open);
    CHECK(gate.reason==core::activation_reason::blocked);
    gate=core::evaluate_operator_activation(features,core::operator_id::post_hdr,activation);
    CHECK(gate.state==core::island_state::fail_open);
    CHECK(gate.reason==core::activation_reason::blocked);

    // Capture-free graph identity: matching pass id 0x04 is insufficient.
    // The second PTDE 0x04 invocation is a Q8 scene -> RGBA 256x180 color
    // copy producer. Stock DSR's depth-only 0x04 must never substitute for it.
    ...
    graph=ready_bloom_graph();
    graph.second_pass04_source_is_q8_scene=false;
    CHECK(validate_bloom_legacy_graph_carrier(graph)==
          bloom_legacy_graph_result::pre_brightpass_copy_edge_not_closed);
    ...
    CHECK(bp.state==post_unblock_state::blocked);
    CHECK(bp.reason==bloom_unblock_reason::packed_depth_quantization_not_closed);
    bloom=ready_bloom();
    bloom.six_pass_graph_insertion_ready=false;
    bp=evaluate_bloom_unblock_preflight(bloom);
    CHECK(bp.reason==bloom_unblock_reason::six_pass_graph_insertion_not_ready);
    bloom=ready_bloom();
    ...
    hp=evaluate_hdr_unblock_preflight(hdr);
    CHECK(hp.reason==hdr_unblock_reason::sfx_inverse_tonemap_contract_not_preserved);
    hdr=ready_hdr();
    hp=evaluate_hdr_unblock_preflight(hdr);
