...
int main()
{
    auto out=locate_fixed_local_specular_material_samples(nullptr,0u);
    CHECK(out.result==
    ...
    out=locate_fixed_local_specular_material_samples(
        unrelated.data(),unrelated.size());
    CHECK(out.result==
        fixed_local_specular_material_sample_result::
            pass_not_fixed_local_specular);

    std::cout<<"fixed_local_specular_material_samples_tests: PASS\n";
    return 0;
}
