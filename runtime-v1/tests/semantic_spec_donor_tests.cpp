...
int main()
{
    ...
    const auto m=find_semantic_spec_donor_override(
        ...
        "a557affd0cddeea208bdb3f1208a5a9bafd7a1dc20f7e5d194b827046c35da59");
    assert(a>=0 && m>=0 && a!=m);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(a)].c102==7.0f);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(m)].c102==8.5f);
    ...
    const auto ll=find_semantic_spec_donor_override(
        ...
        "0f8ca2b6896c9fbe81bfdb30eb0d53a48670cb4c1719ab35533a05f03c46cc00");
    assert(ld>=0 && ll>=0 && ld!=ll);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(ld)].c101[0]==0.25f);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(ll)].c101[0]==0.5f);
    ...
    const auto sw=find_semantic_spec_donor_override(
        ...
        "28610066d4831bc7a3b4b44c3dff33bbd7cfc18b87ebaabfa5c7c0c95846e513");
    assert(lw>=0 && sw>=0 && lw!=sw);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(lw)].c102==25.0f);
    assert(k_semantic_spec_donor_overrides[static_cast<std::size_t>(sw)].c102==50.0f);

    assert(find_semantic_spec_donor_override(
        L"P_Damage_L_Wet[DSB].mtd",
