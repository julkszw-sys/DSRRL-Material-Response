...
bool ensure_shader_pair(device *d,const plan &p,std::span<const std::uint8_t> stock)
{
    ...
    // V9A is a distinct exact P_Metal operator island. Materialize it only on
    // the three owner-validated stable alternates; every other material keeps
    // the ordinary MR shader above.
    if(i>=9u && i<=11u){
...
bool on_create_pipeline(device *d,pipeline_layout,std::uint32_t count,const pipeline_subobject *sub)
{
    try {
        ...
        // order. Ordinary and Lerp shaders are materialized once when DSR
        // creates their exact stock identities.
        ...
        if(const auto *p=find_lerp_plan(ps->code_size,sha)){
            if(!ensure_lerp_shader_pair(d,*p,{bytes,ps->code_size})){
                // Lerp is an additive Build151 coverage extension. A bad exact
                // identity or transform fails open only for this shader.
                ++g_fail_open;
                return false;
            }
            std::lock_guard lock(g_pending_mutex);
